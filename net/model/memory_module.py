...
if __name__=="__main__":
    print("memory net ")
    x=torch.randn(size=[10,10,2,24,24]).cuda()
    model=MemModule(mem_dim=2000,fea_dim=10).cuda()
    # out=model(x)
    # a=torch.sum(out["att"][0])
    # for i in range(out["att"].size(0)):
    #     print(torch.sum(out["att"][i]))
